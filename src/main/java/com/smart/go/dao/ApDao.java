...
public interface ApDao extends JpaRepository<Ap, String> {
    ...
    // 根据建筑名字查询所有楼层
    @Query(value = "SELECT DISTINCT(floor_id) FROM go.ap_info WHERE building_name=?1 ORDER BY floor_id", nativeQuery = true)
    List<String> getLayers(String buildingName);

    // 根据建筑名字，楼层 查询所有房间
    @Query(value = "SELECT DISTINCT(room_id) FROM `ap_info` WHERE (building_name = ?1 and floor_id=?2)  ORDER BY room_id", nativeQuery = true)
